...
getgenv().Use_Is_marco = Decode.Use_Is_marco 
getgenv().PortalSelectTier = Decode.PortalSelectTier 
...
Use_Is_marco = getgenv().Use_Is_marco ,
...
if getgenv().Gems_data == nil then 
getgenv().Gems_data = 0 end 
if getgenv().GenmLimit == nil then 
getgenv().GenmLimit = 0 end 
if getgenv().PackageSet == nil then 
getgenv().PackageSet = "ไม่มีรายการ"  end 
if getgenv().PortalSelectTier == nil then 
getgenv().PortalSelectTier = {} end 
if getgenv().Portal_Ignore_World == nil then
getgenv().Portal_Ignore_World = {} end 
if getgenv().Portal_Ignore_difficulty == nil then 
getgenv().Portal_Ignore_difficulty = {} end 
if getgenv().Portal_Ignore_Weakness == nil then 
getgenv().Portal_Ignore_Weakness = {} end
...
for i,v in pairs(getgc(true))do
	if type(v) == "table" and rawget(v,"item_id") then 
		if string.find(v.item_id,"portal") and v.uuid then 
			portal_data = v 
		end
	end
end

repeat task.wait() until type(portal_data) == "table"

for i,v in pairs(getgc(true))do
	if type(v) == "table" then 
		for x,y in pairs(v)do
			if y == portal_data then 
...
local  Marco2 = create.CreateTab({Title = "Marco"})
...
local Page20 = Test.CreatePage({
    Title = "Portal"
})

mu16x = Page20.Dropdown({
	Title = "Select Portal", 
	Mode = false, 
	Item = {"Alien Portal","Demon Leader's Portal","Puppet Portal","Witch City Portal"}, 
	Default = getgenv().PortalSelectmain, 
	callback = function(v)
		getgenv().PortalSelectmain = v
        Json_Update_data()
	end,
})
...
md1x = Page20.Dropdown({
	Title = "Select Tier", 
	Mode = true, 
	Item = package_Variables[15],
	Default = getgenv().PortalSelectTier, 
	callback = function(v)
		getgenv().PortalSelectTier = v 
        Json_Update_data()
	end,

})

md2x = Page20.Dropdown({
	Title = "Ignore World", 
	Mode = true, 
	Item = Package_World,
	Default = getgenv().Portal_Ignore_World, 
	callback = function(v)
		getgenv().Portal_Ignore_World = v 
        Json_Update_data()
	end,

})
md3x = Page20.Dropdown({
	Title = "Ignore difficulty", 
	Mode = true, 
	Item = {"shield_enemies","regen_enemies","tank_enemies","short_range","fast_enemies","double_cost","triple_cost","hyper_regen_enemies","hyper_shield_enemies","godspeed_enemies","flying_enemies","mini_range"},
	Default = getgenv().Portal_Ignore_difficulty, 
	callback = function(v)
		getgenv().Portal_Ignore_difficulty = v 
        Json_Update_data()
	end,

})

md5x = Page20.Dropdown({
	...
	Default = getgenv().Portal_Ignore_Weakness, 
	callback = function(v)
		getgenv().Portal_Ignore_Weakness = v 
        Json_Update_data()
	end,
...
t29ab = Page20.Toggle({
	...
	Desc = "This function if you use will auto join select portal.", 
	Default = getgenv().Auto_Join_Portal_main, 
	callback = function(v)
        getgenv().Auto_Join_Portal_main = v
        Json_Update_data()
	end,
...
--[[local Page15x = Marco.CreatePage({
...
local Page15 = Marco2.CreatePage({
    Title = "Marco System"
})
...
mu15 = Page15.Dropdown({
	...
	Mode = false, 
	Item = List_files(), 
	Default = getgenv().Use_Is_marco, 
	callback = function(v)
        getgenv().Use_Is_marco  = v
	...
	callback = function()
        if not isfile("Code X Hub/Anime Adventures/Marco/"..marco_file_create..".json") then
            ...
            if isfile("Code X Hub/Anime Adventures/Marco/"..marco_file_create..".json") then
                mu15.Clear()
                for i,v in pairs(List_files())do
                    mu15.Add(v)
                end 
            end  
        end
...
t30 = Page15.Toggle({
	Title = "Marco Record", 
	Desc = "Tower location record, upgrade record, sales record,tower mode record all of you", 
...
t31 = Page15.Toggle({
	Title = "Marco Play", 
	Desc = "Play selected configuration", 
	Default = getgenv().Play_Marco, 
	callback = function(v)
        getgenv().Play_Marco = v
        Json_Update_data()
...
textb4 = Page13.TextBox({
	Title = "Package Set", -- ชื่อ TextBox
	Holder = getgenv().PackageSet, -- คำของ TextBox
	callback = function(v)
        getgenv().PackageSet = v
        Json_Update_data()
	end,
})
...
textb2 = Page13.TextBox({
	Title = "Gems Farm Limit", -- ชื่อ TextBox
	Holder = tostring(getgenv().GenmLimit), -- คำของ TextBox
	callback = function(v)
        getgenv().GenmLimit = tonumber(v)
        Json_Update_data()
	end,
})
textb3 = Page13.TextBox({
	Title = "Gems Data Set", -- ชื่อ TextBox
	Holder = tostring(getgenv().Gems_data), -- คำของ TextBox
	callback = function(v)
        getgenv().Gems_data = tonumber(v)
        Json_Update_data()
	end,
})
...
bu2 = Page13.Button({
	Title = "De Data", -- ชื่อ Button
	DescMode = false, -- เปลี่ยนโหมด ให้ปุ่ม มี Description
	Mode = false, -- เปลี่ยนเป็น Toggle
	callback = function()   
        getgenv().Gems_data = 0 
        getgenv().GenmLimit = 0
        getgenv().PackageSet = "ไม่มีรายการ"
        Json_Update_data()
	end,
})
t26 = Page13.Toggle({
            ...
            ["embeds"] = {
                {
                    ...
                    .."```" ..
                    "\n```md\n".."# ข้อมูลแพ็คเกจ \n"..
                    "📦 แพ็คเกจการฟาม : "..tostring(getgenv().PackageSet).."\n"..
                    "💎 จํานวณเพรชทั้งหมดที่ต้องได้รับ : "..tostring(getgenv().GenmLimit).." Gems".."\n"..
                    "💎 ได้รับเพรชแล้วจํานวณ : "..tostring(getgenv().Gems_data).." Gems" .."\n".."```" ..
...
spawn(function()
    while wait(.5) do 
        pcall(function()
            ...
            if getgenv().Auto_Join_Portal_main then
                if getgenv().PortalSelectmain == "Alien Portal" then 
                    for i,v in pairs(portal_data_main)do
                        if v.uuid and v.item_id == "portal_boros_g" then 
                                    ...
                                    for x1,y1 in pairs(y)do
                                        for x2,y2 in pairs(y1)do
                                             if tostring(x2) == "damage_type" then  
                                                if not table.find(getgenv().Portal_Ignore_Weakness,tostring(y2)) then
                ...
                elseif getgenv().PortalSelectmain == "Demon Leader's Portal" then 
                    for i,v in pairs(portal_data_main)do
                ...
                elseif getgenv().PortalSelectmain == "Puppet Portal" then 
                    ...
                    elseif getgenv().PortalSelectmain == "Witch City Portal" then 
                    for i,v in pairs(portal_data_main)do
                        if v.uuid and v.item_id == "portal_item__madoka" then 
                            for x,y in pairs(v._unique_item_data._unique_portal_data)do
                                    if tostring(x) == "_weak_against"  then
                                        for x1,y1 in pairs(y)do    
                                            for x2,y2 in pairs(y1)do
                                                if tostring(x2) == "damage_type" then     
                                                    ...
                                                    elseif string.find(v._unique_item_data._unique_portal_data.level_id,"aot") then 
                                                        portal_world = "Shiganshinu District"
                                                    elseif string.find(v._unique_item_data._unique_portal_data.level_id,"demonslayer") then 
                                                        portal_world = "Snowy Town" 
                                                    elseif string.find(v._unique_item_data._unique_portal_data.level_id,"naruto") then 
                                                        portal_world = "Hidden Sand Village" 
                                                    elseif string.find(v._unique_item_data._unique_portal_data.level_id,"marineford") then 
                                                        portal_world = "Marine's Ford"   
                                                    elseif string.find(v._unique_item_data._unique_portal_data.level_id,"tokyoghoul") then 
                                                        portal_world = "Ghoul City" 
                                                    elseif string.find(v._unique_item_data._unique_portal_data.level_id,"hueco") then 
                                                        portal_world = "Hollow World" 
                                                    elseif string.find(v._unique_item_data._unique_portal_data.level_id,"hxhant") then 
                                                        portal_world = "Ant Kingdom" 
                                                    elseif string.find(v._unique_item_data._unique_portal_data.level_id,"magnolia") then 
                                                        portal_world = "Magic Town" 
                                                    elseif string.find(v._unique_item_data._unique_portal_data.level_id,"jjk") then 
                                                        portal_world = "Cursed Academy" 
                                                    elseif string.find(v._unique_item_data._unique_portal_data.level_id,"clover") then 
                                                        portal_world = "Clover Kingdom" 
                                                    elseif string.find(v._unique_item_data._unique_portal_data.level_id,"opm") then 
                                                        portal_world = "Alien Spaceship" 
                                                    elseif string.find(v._unique_item_data._unique_portal_data.level_id,"7ds") then 
                                                        portal_world = "Fabled Kingdom" 
                                                    elseif string.find(v._unique_item_data._unique_portal_data.level_id,"mha") then 
                                                        portal_world = "Hero City" 
                                                    elseif string.find(v._unique_item_data._unique_portal_data.level_id,"dressrosa") then 
                                                        portal_world = "Puppet Island" 
...
local Gem_Index  = package_Variables[8].PlayerGui.ResultsUI.Holder.LevelRewards.ScrollingFrame.GemReward.Main.Amount
    ...
    if getgenv().Gems_data == 0 or getgenv().Gems_data == nil then 
        getgenv().Gems_data = tonumber(a_gem)   
        Json_Update_data()
        else 
        getgenv().Gems_data = getgenv().Gems_data + tonumber(a_gem)  
        Json_Update_data()
    end 
end)
...
if string.find(_G.MapName,"Planet Namak") then
 ...
 elseif  string.find(_G.MapName,"Cape Canaveral") then
  ...
  elseif  string.find(_G.MapName,"Fabled Kingdom") then  
        ...
        elseif string.find(_G.MapName,"Puppet Island") then    
            local Part1 = Instance.new("Part")
            local Part2 = Instance.new("Part")
            Part1.Name = "1"
            Part2.Name = "2"
            ...
            Part1.Transparency = 1
            Part2.Transparency = 1
            Part1.CFrame = CFrame.new(8.699502944946289, 10.436573028564453, -168.83106994628906)
            Part2.CFrame = CFrame.new(55.47732162475586, 8.81222915649414, -137.9102020263672)
            local Part1_g = Instance.new("Part")
            local Part2_g = Instance.new("Part")
            local Part3_g = Instance.new("Part")
            local Part4_g = Instance.new("Part")
            ...
            Part1_g.CFrame = CFrame.new(24.935165405273438, 2.600001573562622, -164.39785766601562)
...
-- Cape
Cape_ec = {
    [1] =  CFrame.new(-48.9158935546875, 14.904391288757324, -523.9707641601562),
    [2] =  CFrame.new(-62.12794876098633, 14.904391288757324, -524.21875),
    [3] =  CFrame.new(-55.60767364501953, 14.904391288757324, -535.53271484375) ,
...
Al_ec = {
    [1] =  CFrame.new(-258.9597473144531, 361.2119445800781, 1405.794677734375),
   [2] = CFrame.new(-249.755859375, 361.2119445800781, 1389.0089111328125),
   [3] = CFrame.new(-264.1686096191406, 361.2119445800781, 1366.1680908203125),
   [4] = CFrame.new(-288.9458312988281, 361.2119445800781, 1363.90869140625)
}
...
Puppet_ec_1 = {
    [1] = CFrame.new(-79.01971435546875, 2.59999942779541, -115.97093200683594),
    [2] = CFrame.new(-78.31439208984375, 2.5999996662139893, -123.26460266113281),
    [3] = CFrame.new(-78.22142028808594, 2.5999999046325684, -131.03521728515625),
    [4] = CFrame.new(-63.958946228027344, 2.599998950958252, -104.62680053710938)
}

Puppet_A_1 = {
    [1] = CFrame.new(8.699502944946289, 10.436573028564453, -168.83106994628906),
    [2] = CFrame.new(12.125832557678223, 10.436573028564453, -168.40924072265625),
    [3] = CFrame.new(15.382095336914062, 10.436573028564453, -168.2940216064453),
    [4] = CFrame.new(7.574613571166992, 10.436573028564453, -171.76632690429688),
    [5] = CFrame.new(15.0588960647583, 10.436573028564453, -171.49688720703125),
    [6] = CFrame.new(18.799463272094727, 10.436573028564453, -168.0605010986328),
    [7] = CFrame.new(18.563732147216797, 10.436573028564453, -171.8134765625),
    [8] = CFrame.new(7.446311950683594, 10.436573028564453, -175.2501678466797),
    [9] = CFrame.new(17.169452667236328, 10.436573028564453, -175.0157470703125),
    [10] = CFrame.new(17.294178009033203, 10.43657398223877, -178.50173950195312)  ,
    [11] =   CFrame.new(17.296733856201172, 10.43657398223877, -181.88929748535156),
    [12] =   CFrame.new(20.82883071899414, 10.43657398223877, -177.4658203125) 
}
Puppet_A_2 = {
    [1] = CFrame.new(55.47732162475586, 8.81222915649414, -137.9102020263672),
    [2] = CFrame.new(52.1373291015625, 8.81222915649414, -137.7236785888672),
    [3] = CFrame.new(55.78488540649414, 8.81222915649414, -134.75892639160156),
    [4] = CFrame.new(52.604637145996094, 8.81222915649414, -134.50674438476562)
}
Puppet_G_1 = {
    [1] = CFrame.new(24.935165405273438, 2.600001573562622, -164.39785766601562),
    [2] = CFrame.new(28.276336669921875, 2.600001573562622, -164.61781311035156),
    [3] = CFrame.new(27.22618865966797, 2.600001335144043, -161.33319091796875),
    [4] = CFrame.new(24.637420654296875, 2.600001096725464, -157.1515350341797),
    [5] = CFrame.new(29.788726806640625, 2.600001335144043, -159.4075164794922),
    [6] = CFrame.new(30.6357421875, 2.600001096725464, -156.33119201660156),
    [7] = CFrame.new(30.554039001464844, 2.6000008583068848, -153.2574920654297),
    [8] = CFrame.new(25.67267608642578, 2.6000008583068848, -153.83987426757812),
    [9] = CFrame.new(25.523239135742188, 2.6000008583068848, -150.17030334472656),
    [10] =  CFrame.new(22.083755493164062, 2.6000008583068848, -150.29098510742188)  ,
    [11] =   CFrame.new(25.635055541992188, 2.6000006198883057, -146.69049072265625),
    [12] =   CFrame.new(21.975685119628906, 2.6000006198883057, -146.76348876953125) 
}
Puppet_G_2 = {
    [1] = CFrame.new(49.62041473388672, 2.6000008583068848, -149.64883422851562),
}
Puppet_G_3 = {
    [1] = CFrame.new(84.64189147949219, 2.6000008583068848, -149.52719116210938),
}
Puppet_G_4 = {
    [1] = CFrame.new(106.7662353515625, 2.600001573562622, -170.0411376953125),
}
...
elseif string.find(_G.MapName,"Puppet Island") then  
    ecnomyunit_pos = Puppet_ec_1
    getgenv()["hill_1"] = Puppet_A_1
    getgenv()["hill_2"] = Puppet_A_2
    getgenv()["ground_1"] = Puppet_G_1
             ...
             if not string.find(_G.unit_1,"speedwagon") and not string.find(_G.unit_1,"bulma") then    
                ...
                for i,v in pairs(game:GetService("Workspace")["_terrain"].ground:GetDescendants())do
                    if v:IsA("BasePart") then
                        ...
                        if spawn_cap_1 < _G.unit_1_maxcap  then
                        spawn(function() args = {[1] = getgenv().Unitslotonecode,[2] = _G.unit_target_cc.CFrame * CFrame.new(math.random(1,5),-Check_Pos_Y,-3)}
...
spawn(function()
    while wait() do
        if spawn_ping_delay then 
            wait(.3)
            spawn_ping_delay = nil 
        end
    end
end)
-------------- Play System -------------- 
...
if _G.money_In_Game >= tonumber(Upgrade_Cost_target) then
  ...
  elseif getgenv().Play_type == "Hybrid Index" then   
    -- print("YPn")
    if getgenv().Type[play_step] == "spawn_unit" then 
    --print("YP")
    if string.find(getgenv().Unit_data[play_step],tostring(getgenv().Unitslotone_unit_id)) then
...
Getmethod == "InvokeServer" and self.Name == "spawn_unit"  and spawn_ping_delay == nil then
_G.UnitPos = tostring(Args[2].Position)     
spawn_ping_delay = true 
elseif  getgenv().Record_Marco and 
Getmethod == "InvokeServer" and self.Name == "spawn_unit"  and spawn_ping_delay == true then
return nil
...
local add_json
add_json = package_Variables[1]["_UNITS"].ChildAdded:Connect(function(chil) pcall(function()
if getgenv().Record_Marco then
chil:WaitForChild("_stats")
chil:WaitForChild("_stats"):WaitForChild("uuid")
chil:WaitForChild("_stats"):WaitForChild("id")
...
--[[v._unique_item_data._unique_portal_data.level_id
getgenv().PortalSelectTier
getgenv().Portal_Ignore_World
getgenv().Portal_Ignore_difficulty
getgenv().Portal_Ignore_Weakness]]
...
getgenv().update_marco_file_Gen = function() -- update_marco_file
 ...
 getgenv().update_marco_file = function() -- update_marco_file
        ...
        writefile("Code X Hub/Anime Adventures/Marco/"..getgenv().Use_Is_marco..".json", is_json_marco)
    end    
